...
##    https://urldefense.proofpoint.com/v2/url?u=https-3A__www.researchgate.net_publication_292629179&d=DwIGAw&c=sJ6xIWYx-zLMB3EPkvcnVg&r=IrHkU7fxgRceYNdeCFncvFFNuhguCYv8ta8BbTlpSu0&m=j_SybH0tFtkxgzMfXCf5R3hojU85BTo_iUrkBidntnc&s=PDUBeA5muSQEjhJWrrr_PyUqoDgWJ_LqKEtWaq8hg-4&e= 
...
#--      
      ...
      for(i in id){
        k<-k+1
        plotSegLme(obj, id=i, pop=pop, res=res, xLab="", yLab="", main="",  
              yaxt="n", leg=leg, yscale=yscale, vline=vline, ...)
        #tt<-axTicks(1)       las=2
      #  if(k %in% id.bot) axis(1, cex.axis=.7, at=NULL) else axis(1, labels=FALSE)
        if(k %in% id.sx) axis(2, labels=TRUE, cex.axis=.9) else axis(2, labels=FALSE)
        }
